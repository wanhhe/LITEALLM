    ...
    def download(self) -> bool:
        ...
        if not os.path.exists(librispeech_dir):
            ...
            try:
                ...
                download_success = os.system("wget --no-check-certificate https://us.openslr.org/resources/12/train-clean-100.tar.gz -O train-clean-100.tar.gz")
                if download_success != 0:
                    raise RuntimeError("Failed to download train-clean-100 dataset")
                # Download train-clean-360 dataset
                download_success = os.system("wget --no-check-certificate https://us.openslr.org/resources/12/train-clean-360.tar.gz -O train-clean-360.tar.gz")
                if download_success != 0:
                    raise RuntimeError("Failed to download train-clean-360 dataset")
                # Download train-other-500 dataset
                download_success = os.system("wget --no-check-certificate https://us.openslr.org/resources/12/train-other-500.tar.gz -O train-other-500.tar.gz")
                if download_success != 0:
